...
function Cancelation({
    ...
    const handleCancel = () => {
        Axios.get(
            `${process.env.REACT_APP_BASE_URL}fan/cancel/reservation?stadium_id=${seatInfo.stadiumId}&match_id=${seatInfo.matchId}&row=${seatInfo.row}&col=${seatInfo.col}`,
            {
...
function Reservation({
    ...
    const handlePurchase = () => {
        setLoading(true);
        Axios.get(
            `${process.env.REACT_APP_BASE_URL}fan/reserve?stadium_id=${seatInfo.stadiumId}&match_id=${seatInfo.matchId}&row=${seatInfo.row}&col=${seatInfo.col}`,
            {
                headers: { Authorization: "Bearer " + getToken() }
            }
        ).then(res => {
...
export default function Home() {
    ...
    useEffect(() => {
        if (getToken() !== null)
            ...
            Axios.get(process.env.REACT_APP_BASE_URL + "matches").then(res => {
                setMatches(res.data.matches);
            });
        }
    }, []);
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            if (matchId !== -1) {
                ...
                Axios.get(
                    process.env.REACT_APP_BASE_URL + "match" + "/" + matches[matchId].id,
                    headersObj
                )
                    .then(res => {
