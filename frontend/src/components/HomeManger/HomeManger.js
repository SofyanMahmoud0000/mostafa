...
export default function HomeManger() {
  ...
  const handleSubmit = () => {
    if (dialogOptions.type === "Edit") {
      Axios.post(
        process.env.REACT_APP_BASE_URL + "manager/update/match/" + dialogOptions.data.id.toString(),
        {
      ...
      Axios.post(
        process.env.REACT_APP_BASE_URL + "manager/add/match/",
        {
          home_team_id: dialogOptions.data.home_team.id,
          away_team_id: dialogOptions.data.away_team.id,
          stadium_id: dialogOptions.data.stadium.id,
      ...
      Axios.post(
        process.env.REACT_APP_BASE_URL + "manager/add/stadium/",
        {
  ...
  useEffect(() => {
    ...
    Axios.get(process.env.REACT_APP_BASE_URL + "manager/stadiums/", {
      headers: { Authorization: "Bearer " + getToken() }
    }).then((res) => {
    ...
    Axios.get(process.env.REACT_APP_BASE_URL + "manager/teams/", {
      headers: { Authorization: "Bearer " + getToken() }
    }).then((res) => {
      if (res.data.teams) {
